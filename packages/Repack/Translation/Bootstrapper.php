...
class Bootstrapper
{
    ...
    {
        if ($ioc->offsetExists('translator') && method_exists($ioc->bound('translator'), 'addJsonPath')) {return;}

        //1. Bind the translation line loader.
        static::bindLoader($ioc);

        //2. Bind the translator.
        $ioc->singleton('repack.translator', function ($ioc) {
    ...
    {
        $ioc->singleton('repack.translation.loader', function () use ($ioc) {
    ...
    {
        $translator instanceof TranslatorContract ?: $translator = $translator['translator'];
